<template>
    <div>
       <router-link class='home-link' :to="{ name: 'Home' }">
        ...
        <div class="form">
            <input type="text" v-model="todo.title" placeholder="..." class="form__header_input">
...
export default {
    ...
    data() {
        return {
            ...
            todo: this.$store.state.editTodo
        }
    },
    ...
    methods: {
        addTask() {
            ...
            this.todo.list.push({id: new Date().getTime(), text: this.inputText, checked: false})
            this.inputText = ''
        },
        removeTask(task) {
            this.todo.list = this.todo.list.filter(el => el.id != task.id)
        },
        createTodo() {
            if (this.todo.list.length == 0) return
            this.todo.id = new Date().getTime()

            this.$store.commit('setEditTodo', this.todo)
            this.$store.commit('setCreateTodo', false)
